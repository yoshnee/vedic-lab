/* ============================================================
   ...
   Plain JS (no JSX); returns SVG markup strings.
...
(function () {
  ...
  /* Planet body SVG (neutral). key e.g. 'sun'. retro adds an "R" superscript. */
  function body(key, size, retro) {
    _u++;
    var marker = retro
      ? '<g><circle cx="39.5" cy="9" r="7.6" fill="#0E1B12" fill-opacity="0.5"/><text x="39.5" y="9" dy="0.34em" text-anchor="middle" font-family="Space Mono, monospace" font-weight="700" font-size="11.5" fill="#E7C977">R</text></g>'
      : '';
    return '<svg viewBox="0 0 48 48" width="' + size + '" height="' + size + '" style="display:block">'
      + bodyInner(key, _u) + marker + '</svg>';
  }

  /* North Indian chart-diamond mark with a glowing point of light.
     gold on dark — used for deck backs & the logo. */
  ...
  /* Diamond medallion enclosing a short label — generic icon
     (used for Houses; reusable for any numeric/short glyph). */
